...
- Within the 'Listen for' field, the `*` is a throw-away, catch-all character. It's purpose is for matching miscellaneous phrases, not for capturing text. For example, let's say that you've put "* volume $" in your 'Listen for' field, and `change_audio_volume.py "$"` in your Parameters field.
